...
description: >-
  PR 番号を引数に取り、diff + description を Codex に投げてレビューさせる。 PR 作成直後に必ず実行する (詳細は
  .rulesync/rules/pr-review-policy.md)。 "PR レビュー", "pr-codex-review", "Codex で
  PR レビュー", "PR をレビューして" 等で発動。
---
# PR Codex Review
...
Codex 出力には PROMPT 内の `## Summary` 文字列も含まれるため、**最後の `## Summary` セクション**だけを抽出する。
`tac` で末尾から逆引きする方式で統一する (動作が単純で確実):
