...
description: >-
  設計書から実装タスク Issue を分割起票する。"設計書から issue 化", "実装タスクを起票", "issue-from-spec",
  "設計書からチケット分割" 等で発動。
---
# 設計書 → Issue 分割起票スキル
...
承認後、各 Issue を `gh issue create` で起票する。
...
4. **依存の循環**: 前提 Issue が循環していないこと
5. **適用範囲の存在**: 実装プロンプト名から判定したパス (`core/`, `examples/...`) が実在すること
