...
> 最終更新: 2026-05-09 (M1.1: OIDC OP 規約節を新設、設計 #32)
...
| Path                                | Method | 認証 | 概要                                                                                             | 追加マイルストーン |
| ----------------------------------- | ------ | ---- | ------------------------------------------------------------------------------------------------ | ------------------ |
| `/health`                           | GET    | 不要 | サーバー稼働確認 (`{"status":"ok"}`)                                                             | M0.1               |
| `/health/live`                      | GET    | 不要 | プロセス疎通 (DB 非依存、k8s livenessProbe 想定)                                                 | M0.3               |
| `/health/ready`                     | GET    | 不要 | DB Ping with 2s timeout、200 / 503 (k8s readinessProbe 想定)                                     | M0.3               |
| `/.well-known/openid-configuration` | GET    | 不要 | OIDC Discovery 1.0 / RFC 8414 メタデータ (公開、F-1〜F-4 / F-16)                                 | M1.1               |
| `/jwks`                             | GET    | 不要 | JWKS (RFC 7517) — Q9: 拡張子なし / 大手準拠の `application/json` (論点 #7)                       | M1.1               |
| `/authorize`                        | GET    | 不要 | M1.2 で本実装。M1.1 は 503 + `{"error":"endpoint_not_implemented","available_at":"M1.2"}` (F-23) | M1.1 (stub)        |
| `/token`                            | POST   | 不要 | M1.3 で本実装。M1.1 は 503 + `{"available_at":"M1.3"}` (同上)                                    | M1.1 (stub)        |
| `/userinfo`                         | GET    | 不要 | M1.4 で本実装。M1.1 は 503 + `{"available_at":"M1.4"}` (同上)                                    | M1.1 (stub)        |

## OIDC OP 規約 (M1.1 で導入、設計 #32)

OIDC OP として外部 RP に公開するメタデータ + 鍵管理の規約 (F-19 計 8 項目)。

### 1. issuer URL 規約

- `CORE_OIDC_ISSUER` env で必須。`https://` 必須 (`CORE_ENV=dev` のみ `http://` 許可)
- 末尾 `/` は config.Load() で strip される (`https://example.com/` → `https://example.com`)
- subpath issuer (`https://example.com/id-core`) でも各 endpoint URL は `url.URL.JoinPath` で正しく構築 (F-3 / F-17)

### 2. 環境変数一覧 (M1.1 追加分)

`docs/context/backend/registry.md` の「環境変数一覧」を参照。要点: `CORE_ENV` は **strict 3 値** (`prod` / `staging` / `dev`)、それ以外で起動失敗。`CORE_OIDC_KEY_FILE` は `prod` で必須、`staging`/`dev` は `CORE_OIDC_DEV_GENERATE_KEY=1` で代替可能。

### 3. 鍵保管方式 + kid と fingerprint の関係 + 鍵長ガイダンス (F-7 / F-11 / F-18 / 論点 #16)

| 環境        | 方式                                                                |
| ----------- | ------------------------------------------------------------------- |
| 本番        | `CORE_OIDC_KEY_FILE` で K8s Secret マウントパス (PEM PKCS#8) を指定 |
| dev/staging | `CORE_OIDC_DEV_GENERATE_KEY=1` で起動時 RSA 2048 bit メモリ生成     |

**kid 算出 (F-11)**: 公開鍵 DER (`x509.MarshalPKIXPublicKey`) の SHA-256 → 先頭 12 バイト → 24 hex 文字。**RFC 7638 JWK Thumbprint 非準拠** (id-core 独自仕様)。`CORE_OIDC_KEY_ID` 設定時はその値が優先。

**ログ表記**: 「kid」または「fingerprint」を用い、「thumbprint」は使わない (RFC 7638 と混同しないため)。

**鍵長ガイダンス**: 任意 bit 数の RSA 鍵を受け入れる (鍵長透過、env で強制しない)。推奨は **2048 bit 以上 (RS256 下限)、4096 bit まで実用範囲、>4096 bit は HTTP timeout 接近リスクあり**。1024 bit 以下は受け入れるが起動 WARN ログを出す。**本番鍵生成は id-core スコープ外** (運用側 = openssl / cloud KMS / HSM 等で生成)、devkeygen は dev 専用 (F-13)。

### 4. dev 鍵生成モードの単一 Pod 制約 (F-8 / Q5)

`CORE_OIDC_DEV_GENERATE_KEY=1` での起動は **Helm/manifest 側で `replicas: 1` を強制**することが運用側責務。アプリ側は env でレプリカ数を検証しない (責務分界)。複数 Pod で起動するとそれぞれ別鍵を生成し、JWKS 不整合を起こす。実 sample は M1.5 の K8s manifest で整備予定。

### 5. 開発者向け運用手順

```bash
# 1. dev 鍵を生成 (`core/dev-keys/` 配下、.gitignore で除外済)
...
# 3. dev 環境で起動
CORE_ENV=dev \
  CORE_OIDC_ISSUER=http://localhost:8080 \
  CORE_OIDC_KEY_FILE=./dev-keys/signing.pem \
  make -C core run
...
**M1.1 範囲は鍵更新を非サポート**。Secret 内の鍵を変更したい場合は **Pod 全停止 → 新鍵で再起動** の運用フロー必須。M1.1 で rolling update 中に Secret を変更すると、Pod ごとに旧鍵/新鍵が混在し署名検証不整合を起こす。

zero-downtime rotation (overlap window) は M2.x で実装予定。Cache-Control max-age = 5 分の場合の overlap window 実務目安は **15〜30 分以上** (`>= max-age + RP 再取得遅延 + 時計ずれバッファ`)。

### 7. 鍵フォーマット受け入れポリシー (論点 #10 Codex MEDIUM 2)

| 入力                                                    | 挙動                                                                                                                            |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| PKCS#8 PEM (`-----BEGIN PRIVATE KEY-----`)              | OK (受け入れ)                                                                                                                   |
...
| RSA 以外 (EC / Ed25519 等)                              | エラー: 「RS256 では RSA 鍵が必要です。受け取った鍵タイプ: <type>」                                                             |
| 1024 bit 以下                                           | 拒否しないが起動 WARN ログ                                                                                                      |

### 8. JWKS 出力契約 (論点 #10 Codex HIGH 1 / LOW 2)

- jwx/v3 のバージョンは `go.mod` でマイナー / パッチまで実質固定 (Renovate / Dependabot で自動更新する場合も major / minor は手動レビュー必須)
- `alg=RS256` / `kty=RSA` / `use=sig` を明示セット
- private 成分 (`d` / `p` / `q` / `dp` / `dq` / `qi`) は JWKS に出力しない (golden / 契約テストで担保、`core/internal/oidc/jwks/testdata/jwks_golden.json`)
- `Cache-Control: public, max-age=300, must-revalidate` 既定 (`CORE_OIDC_JWKS_MAX_AGE` で 0〜86400 override)
- ETag は SHA-256(body)[:16] → `base64.RawURLEncoding` → ダブルクォート囲み (24 文字 strong ETag、Discovery と同一仕様)
- `immutable` は使わない (M2.x で鍵 rotation 予定のため)
...
bootstrap (config → logger → event_id)
  ...
  → AssertClean (M0.3)
  → keystore.Init  ★ M1.1
  → INFO 起動鍵情報 (kid / alg=RS256 / source=file|generated / env)
  → WARN dev 鍵生成モード (source=generated 時のみ)
  → WARN 鍵長 < 2048 bit (該当時のみ)
  → server.New (cfg, logger, pool, keystore)
  → ListenAndServe
```

各失敗 → `logger.Error` + `os.Exit(1)` (M0.3 既存パターン踏襲、`log.Fatal*` 不使用)。

**F-18 redact**: 秘密鍵 / PEM フルダンプ / RSA modulus (`n`) / exponent (`e`) のログ出力禁止。kid (公開鍵 DER SHA-256 先頭 24 hex) は公開可能な fingerprint として扱う。
