...
6. Secret を更新する場合 (鍵差し替え) は **rolling restart 不可**。**全 Pod を一度停止 → 新鍵で再起動** する運用必須 (rolling 中は旧鍵 / 新鍵が Pod 間で混在し署名検証不整合となるため、F-24 参照)。zero-downtime な多鍵 rotation は M2.x で実装予定
...
| F-23     | M1.1 で広告するが M1.2-1.4 で実装される endpoint (`authorization_endpoint` / `token_endpoint` / `userinfo_endpoint`) は **OIDC Discovery 1.0 で REQUIRED のため Discovery メタデータには必須記載**。実 endpoint は M1.x の各マイルストーン到達まで HTTP **`503 Service Unavailable`** を返し、レスポンス body は機械可読 JSON (`{"error":"endpoint_not_implemented","available_at":"<milestone>"}` 形式、`<milestone>` は endpoint ごとに固定値: `/authorize`=`M1.2` / `/token`=`M1.3` / `/userinfo`=`M1.4`) とする。これにより RP は能力宣言と現状の不一致を確実に判別できる (Codex HIGH 反映) |
...
| 検証性         | F-1〜F-20 を Go 単体テストおよび統合テスト (実機 K8s 不要) で検証可能。アプリ内で実施する検証は **env ガード (`CORE_ENV=prod` + 鍵未指定で起動失敗、F-9) / レスポンス契約 (Discovery / JWKS の JSON 構造、F-1〜F-5) / Cache-Control / ETag 整合 (F-1 / F-6 / F-21) / URL 構築 ContractTest (F-17, Q8 の 5 ケース)** で構成する。複数 Pod 環境のレプリカ数検証はアプリ側責務外 (Q5 / F-8、Helm/manifest 側責任)。CI 上で同等のテストが green になる |
...
| 2026-05-02 | PR #33 Codex レビュー (HIGH=1 / MEDIUM=3) 反映: ユーザーシナリオ運用者 (6) を rolling restart 不可に修正、検証性をアプリ内検証 (env ガード / レスポンス契約 / Cache-Control / ETag / URL ContractTest) に置換、F-23 の `available_at` 例を endpoint ごと出し分け表現に変更、機能要件・変更履歴テーブルの列数不一致 (余分な `\| --- \|` / `\|     \|`) を修正 |
