...
sequenceDiagram
    ...
    RID->>RID: context に request_id 注入 + レスポンスヘッダ X-Request-Id を先行設定
    Note over RID: ヘッダ設定は next 呼び出し前。<br/>WriteHeader 後の変更は反映されないため。
    RID->>AL: next.ServeHTTP(w, r-with-ctx) 呼び出し
    AL->>AL: start := time.Now() / status 記録用 ResponseWriter ラップ
    Note over AL: defer で 1 行アクセスログ出力 (D3)
    AL->>REC: next.ServeHTTP
    ...
    AL-->>RID: 戻り
    RID-->>C: レスポンス完了 (X-Request-Id ヘッダは先行設定済み)
```
...
sequenceDiagram
    ...
    C->>RID: HTTP リクエスト
    RID->>RID: request_id を context に注入 + レスポンスヘッダ X-Request-Id を先行設定
    Note over RID: ヘッダ設定は next 呼び出し前 (図 1 と同じ理由)
...
flowchart TD
    ...
    Sanitize["元値をサニタイズ<br/>(制御文字をエスケープ、長さ切詰め)"] --> LogClient[ログにのみ <br/>client_request_id として記録]
    LogClient --> Gen

    Accept --> Inject[context.Context に request_id を注入]
    Gen --> Inject
    Inject --> RespHeader["レスポンスヘッダ X-Request-Id に<br/>採用値を先行設定 (next 呼び出し前)"]
...
> サニタイズの具体: 制御文字 (`0x00`–`0x1F`, `0x7F`) を Unicode エスケープ表記 (例: `0x0A` → `
`) に置換し、長さを 128 オクテットで切り詰めて `client_request_id` フィールドに記録する。本文上のエスケープ表記は環境差を避けるため flowchart ラベル内では平文に書き、F-6 の本文と本注釈で詳細を補足する。
...
| 11. 実装プロンプト生成 | 完了       | `/issue-from-spec` で実装 Issue 3 件を起票: #12 (logger + apperror) → #13 (middleware + server) → #14 (契約テスト + context + Makefile)。依存関係 A → B → C                                                                                   |
...
| 2026-05-02 | Codex PR レビュー (PR #11) 指摘反映 (HIGH 1 / MEDIUM 2 / LOW 1): 図 1/2 で `X-Request-Id` レスポンスヘッダ付与を **next 呼び出し前** に統一 (handler が `WriteHeader` を呼ぶと以降のヘッダ変更が無視されるため)。図 2 の `H-xH` 自己 cross-arrow を `H->>H: panic 発生` + Note に保守化。図 3 の flowchart ラベル内 `\u00XX` を平文に置き換え、注釈で詳細補足                                                                                                                                                                              |
| 2026-05-02 | `/issue-from-spec` 実施: 実装 Issue 3 件を起票し親 Issue #7 に task list 追記。依存関係 A → B → C: #12 (logger + apperror、テスト 27 ケース)、#13 (middleware + server + log.Fatal 全廃、テスト 33 ケース)、#14 (契約テスト + context 4 ファイル + Makefile + README、テスト 5 ケース)。設計書の状態を「着手中」→「完了 (実装フェーズへ移行)」に更新、フェーズ 11 を完了                                                                                                                                                                   |
