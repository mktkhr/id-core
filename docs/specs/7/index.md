...
| Q4  | `time` フィールドのフォーマット                                 | (a) RFC3339Nano (UTC、`Z` suffix) (b) RFC3339 秒精度 (c) Unix epoch (ms)                                                                                    | **(a) RFC3339Nano (UTC、`Z` suffix 強制)**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `log/slog` のデフォルトフォーマット、ナノ秒精度は診断・順序付けに有用、UTC で TZ 混在排除。PostgreSQL `timestamptz` (UTC 内部表現、μs 精度) と完全互換。アプリ起動時 `time.Local = time.UTC` で UTC ベースに統一              |
...
| D1  | middleware の構成順序 (recover / request_id / access_log)       | (a) recover → request_id → access_log → handler (b) request_id → recover → access_log → handler (c) その他                                                  | **(b) `request_id` → `recover` → `access_log` → `handler`**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `request_id` を最外側に置くことで panic 時を含む全ログレコードに `request_id` が付く。UUID v7 生成は実質 panic しないので recover を内側にして問題なし。access_log は recover の内側で status / duration を取得する流れが自然 |
...
│   │   └── access_log_test.go
│   ├── server/
│   │   └── server.go            # ServeMux + middleware チェーン (D1: request_id → recover → access_log → handler)
│   └── health/
...
| Method | Path      | 認証        | 変更内容                                                                         |
| ------ | --------- | ----------- | -------------------------------------------------------------------------------- |
| GET    | `/health` | 不要 (公開) | 外形互換。middleware チェーン (request_id / recover / access_log) を通る変更のみ |

### middleware チェーン仕様

D1 で決定した順序: **`request_id` (最外側) → `recover` → `access_log` → `handler`**。

リクエスト処理フロー:

```
Client → request_id (生成 / 検証 / context 注入) → recover (panic 捕捉) → access_log (status/duration 計測) → handler
                                                  ↑                       ↑
                                              panic 時はここで              終了時にここで 1 行出力
                                              ERROR ログ + 500 応答
```
...
- `time`: RFC3339Nano UTC、`Z` suffix 強制 (Q4)。アプリ起動時に `time.Local = time.UTC` で UTC ベースに統一
...
- `docs/context/backend/registry.md`: `core/internal/{logger,errors,middleware}` の各パッケージを登録
